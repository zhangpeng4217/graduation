...
//    val dataFrame = spark.sql("select * from E_Commerce_Data_Clean").show()
    dataFrame.write.format("jdbc").options(Map("url"->"jdbc:mysql://192.168.137.10:3306/sparksql","user"->"root","password"->"root","dbtable"->"questions01")).mode(SaveMode.Append).save()
    //    关闭环境
    spark.close()
  }
}
